...
st.markdown("""
    ...
    div.stButton > button {
        background-color: #244D3D !important; /* 기본 붉은색 */
...
st.markdown("""
    ...
    /* 버튼 스타일 */
    div.stButton > button {
        background-color: #244D3D !important;
        color: white !important;
        font-weight: bold !important;
        border-radius: 0.5rem !important;
        transition: all 0.3s ease !important;
    ...
    .refresh-title:hover { color: #244D3D !important; }
    </style>
""", unsafe_allow_html=True)
    ...
    if local_logo_url:
        ...
        st.markdown(f'''
            <a href="{inst_url}" target="_blank" style="text-decoration:none; font-weight:bold; font-size:24px; display:block; margin-top:10px;">
                <span style="color:#244D3D;">{inst_abbr}</span> 
...
with st.expander("📚 **관련 KOSHA GUIDE (기술지원규정) 조회 및 다운로드**", expanded=True):
    st.caption("※ 산업안전보건공단 Open API 연동: 법적 최소 기준 외에 현장에 바로 적용 가능한 기술지원규정 원문 제공")
    ...
    if 'edited_df' in locals() and edited_df is not None and not edited_df.empty:
        ...
        # ------------------------------------------------------------------
        st.info("💡 **위험성평가 분석을 실행하시면**, 추출된 핵심 위험요인에 맞춘 기술지원규정(KOSHA GUIDE) 원문 다운로드 링크가 이곳에 자동으로 제공됩니다.")
